/* ------------------------------------------------------------------
   ...
   Shipping cost itself is handled entirely by Square: when the
   customer chooses "Post to me", this asks Square to collect a
   shipping address, and Square applies whichever Shipping Rate
   Profile you've configured in your Square Dashboard (Settings >
   Account & Settings > Fulfillment methods > Shipment) for that
   destination. This site doesn't calculate or add a shipping charge
   itself — see README for how to set up rates on Square's side.
...
exports.handler = async (event) => {
  ...
  let requestedItems, fulfillment;
  try {
  ...
  const payload = {
    ...
    order: {
      ...
      note: fulfillment === "shipping" ? "Ship to customer" : "Collect in store",
    },
    checkout_options: {
