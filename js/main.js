...
function addToCart(id, qty = 1) {
  ...
  cart[id] = Math.min(currentQty + qty, cap);
  saveCart(cart);
  flashCartIcon();
  return cart[id] < currentQty + qty; // true if capped by limited stock
}
...
function sleeveCardHTML(product) {
  ...
  return `
    <article class="sleeve-card" data-genre="${product.genre}" data-tags="${product.tags.join(",")}">
      <a href="product.html?id=${product.id}" class="sleeve ${isSoldOut ? "sleeve--soldout" : ""}" data-accent="${product.accent}">
...
function renderGrid(container, products) {
  ...
  container.querySelectorAll("[data-add]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.preventDefault();
      const wasCapped = addToCart(btn.getAttribute("data-add"), 1);
      btn.textContent = wasCapped ? "Only 1 left" : "Added ✓";
